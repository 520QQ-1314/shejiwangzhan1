/**
 * DesignHub Cloudflare Worker 代理 v1.1
 * 新增 Dribbble 域名白名单
 */
...
async function handle(req) {
  ...
  if (url.pathname === '/' || url.pathname === '/health') {
    return json({ ok: true, name: 'DesignHub Proxy v1.1', allowed: [...ALLOWED_HOSTS] });
  }
  ...
  if (!ALLOWED_HOSTS.has(t.hostname))
    return json({ error: 'host not allowed', host: t.hostname, allowed: [...ALLOWED_HOSTS] }, 403);

  const headers = new Headers();
  for (const [k, v] of req.headers) {
    if (!['host','origin','referer','cf-connecting-ip','cf-ipcountry','cf-ray','cf-visitor','x-forwarded-for','x-forwarded-proto','x-real-ip'].includes(k.toLowerCase())) {
      headers.set(k, v);
    }
  }
